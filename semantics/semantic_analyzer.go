...
func analyzeExpression[A analyzer](a A, expr ast.BLangExpression, expectedType semtypes.SemType) {
	...
	case *ast.BLangRecordLiteral:
		analyzeRecordLiteralExpr(a, expr, expectedType)
...
func analyzeRecordLiteralExpr[A analyzer](a A, expr *ast.BLangRecordLiteral, expectedType semtypes.SemType) {
	if expectedType != nil {
		resultType, mappingAtomicType := selectMappingInherentType(a, expr, expectedType)
		for _, f := range expr.Fields {
			kv := f.(*ast.BLangRecordKeyValueField)
			var keyName string
...
func selectMappingInherentType[A analyzer](a A, expr *ast.BLangRecordLiteral, expectedType semtypes.SemType) (semtypes.SemType, semtypes.MappingAtomicType) {
