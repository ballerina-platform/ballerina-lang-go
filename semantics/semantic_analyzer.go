...
func analyzeBinaryExpr[A analyzer](a A, binaryExpr *ast.BLangBinaryExpr, expectedType semtypes.SemType) {
	...
	} else if isLogicalExpr(binaryExpr) {
		if !semtypes.IsSubtypeSimple(lhsTy, semtypes.BOOLEAN) || !semtypes.IsSubtypeSimple(rhsTy, semtypes.BOOLEAN) {
			a.semanticErr(fmt.Sprintf("expect boolean types for %s", string(binaryExpr.GetOperatorKind())))
			return
		}
	}
...
func analyzeSimpleVariableDef[A analyzer](a A, simpleVariableDef *ast.BLangSimpleVariableDef) {
	...
	if variable.GetName().GetValue() == string(model.IGNORE) {
		if !semtypes.IsSubtypeSimple(expectedType, semtypes.ANY) {
			a.semanticErr("wildcard binding pattern type must be a subtype of 'any'")
			return
		}
	}
