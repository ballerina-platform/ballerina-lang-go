...
func determineObjectType[A analyzer](a A, expr *ast.BLangNewExpression, objectTy semtypes.SemType) (semtypes.SemType, bool) {
	...
	for _, alt := range alts {
		...
		if semtypes.IsSubtype(cx, argListTy, paramListTy) {
			...
			candidates = append(candidates, canditate{objType: alt.ObjectType, initReturnType: retTy})
		}
	}
	if len(candidates) == 0 {
		...
		a.semanticErr("ambigious object type", expr.GetPosition())
		return nil, false
	}
