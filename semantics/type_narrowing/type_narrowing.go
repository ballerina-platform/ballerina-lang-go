...
func (b *binding) isUnnnarowing() bool {
...
func lookup(chain *binding, ref model.SymbolRef) (model.SymbolRef, bool) {
	...
	if chain.ref == ref {
		return chain.narrowedSymbol, !chain.isUnnnarowing()
	}
...
func analyzeWhileStmt(ctx *context.CompilerContext, chain *binding, stmt *ast.BLangWhile) statementEffect {
	...
	if !bodyEffect.nonCompletion {
		result = mergeChains(ctx, result, bodyEffect.binding)
	}
...
func analyzeIfStatement(ctx *context.CompilerContext, chain *binding, stmt *ast.BLangIf) statementEffect {
	...
	return mergeStatmentEffects(ctx, ifTrueEffect, ifFalseEffect)
}

func mergeStatmentEffects(ctx *context.CompilerContext, s1, s2 statementEffect) statementEffect {
	if s1.nonCompletion {
		return s2
	}
	if s2.nonCompletion {
	...
	combined := mergeChains(ctx, s1.binding, s2.binding)
...
func analyzeBinaryExpr(ctx *context.CompilerContext, chain *binding, expr *ast.BLangBinaryExpr) expressionEffect {
	...
	case model.OperatorKind_AND:
		...
		return expressionEffect{
			ifTrue:  rhsEffect.ifTrue,
			ifFalse: mergeChains(ctx, lhsEffect.ifFalse, rhsEffect.ifFalse),
		}
	case model.OperatorKind_OR:
		lhsEffect := analyzeExpression(ctx, chain, expr.LhsExpr)
		rhsEffect := analyzeExpression(ctx, chain, expr.RhsExpr)
		return expressionEffect{
			ifTrue:  mergeChains(ctx, lhsEffect.ifTrue, rhsEffect.ifTrue),
			ifFalse: mergeChains(ctx, lhsEffect.ifFalse, rhsEffect.ifFalse),
		}
	default:
		return defaultExpressionEffect(chain)
...
func mergeChains(ctx *context.CompilerContext, c1 *binding, c2 *binding) *binding {
	...
	for s1, ty1 := range m1 {
		ty2, hasS1 := m2[s1]
		if hasS1 {
			ty := semtypes.Union(ty1, ty2)
			narrowedSymbol := narrowSymbol(ctx, s1, ty)
			result = &binding{
				ref:            s1,
		...
		// Not adding to binding chain has the same effect an unnarowing
	}
	return result
}
...
// FIXME: this should give the narrowed symbol if it is already narrowed
