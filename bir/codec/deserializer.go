...
func (br *birReader) readPackage() (pkg *bir.BIRPackage, err error) {
	...
	br.skipConstants() // constants are now part of global vars
...
func (br *birReader) skipConstants() {
	count := br.readLength()
	for i := 0; i < int(count); i++ {
		_ = br.readStringCPEntry() // name
		_ = br.readFlags()         // flags
		_ = br.readOrigin()        // origin
		_ = br.readPosition()      // pos

		var typeIdx int32
		br.read(&typeIdx)

		bufLen := br.readLength()
		// Skip the constant value bytes
		skip := make([]byte, bufLen)
		br.read(skip)
	}
}
