...
func TestIntegrationQueryExprFromMap(t *testing.T) {
...
public function main() {
	...
	if got != "[2,4,6]" {
		t.Fatalf("unexpected output for map query expression: got %q", got)
	}
}

func TestIntegrationQueryExprConstructMap(t *testing.T) {
	const source = `import ballerina/io;

public function main() {
    [string,int][] pairs = [["a", 1], ["b", 2], ["c", 3]];
    map<int> out = map from var p in pairs
        select [p[0], p[1] * 2];
    io:println(out);
}`
	...
	got := strings.TrimSpace(stdout)
	if got != "{\"a\":2,\"b\":4,\"c\":6}" {
		t.Fatalf("unexpected output for map query construct type: got %q", got)
	}
}

func TestIntegrationQueryExprConstructMapDuplicateKeys(t *testing.T) {
	const source = `import ballerina/io;

public function main() {
    int[] xs = [1, 2, 3];
    map<int> out = map from var x in xs select ["a", x];
    io:println(out["a"]);
}`
	...
	got := strings.TrimSpace(stdout)
	if got != "3" {
		t.Fatalf("unexpected output for duplicate-key map query construct: got %q", got)
	}
}

func TestIntegrationQueryExprConstructMapKeywordRequired(t *testing.T) {
	const source = `import ballerina/io;

public function main() {
    map<int> xs = {"a": 1, "b": 2, "c": 3};
    map<int> out = from var x in xs select ["k", x];
    io:println(out["k"]);
}`
	...
	if strings.TrimSpace(stdout) != "" {
		t.Fatalf("expected empty stdout for compile-time error case, got: %q", stdout)
	}
	if !strings.Contains(stderr, "incompatible type") {
		t.Fatalf("expected incompatible type error when map construct keyword is missing, got: %q", stderr)
	}
}

func TestIntegrationQueryExprConstructMapVarInference(t *testing.T) {
	const source = `import ballerina/io;
...
public function main() {
    int[] xs = [1, 2, 3, 4];
    var out = map from var x in xs select [keyFor(x), x];
    io:println(out["a"]);
    io:println(out["b"]);
}`
	...
	got := strings.TrimSpace(stdout)
	if got != "2\n4" {
		t.Fatalf("unexpected output for map query var inference: got %q", got)
	}
}

func TestIntegrationQueryExprFromMapToMap(t *testing.T) {
	const source = `import ballerina/io;
...
public function main() {
	...
	got := strings.TrimSpace(stdout)
	if got != "10\n20\n30" {
		t.Fatalf("unexpected output for map-to-map query expression: got %q", got)
	}
}
