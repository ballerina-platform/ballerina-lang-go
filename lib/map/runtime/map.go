...
func initMapModule(rt *runtime.Runtime) {
	...
	runtime.RegisterExternFunction(rt, orgName, moduleName, "keys", func(args []values.BalValue) (values.BalValue, error) {
	...
	runtime.RegisterExternFunction(rt, orgName, moduleName, "remove", func(args []values.BalValue) (values.BalValue, error) {
		m := args[0].(*values.Map)
		key := args[1].(string)
		val, _ := m.Get(key)
		m.Delete(key)
