...
5. **`secureSocket` TLS configuration is partially supported**
   Implemented fields: `enable` / `verifyHostName` (both disable verification via `InsecureSkipVerify`); `cert` (string PEM file path → custom CA trust store); `key` (`CertKey` with `certFile`/`keyFile` → mTLS); `serverName` (SNI via `tls.Config.ServerName`); `ciphers` (IANA cipher suite name list → `tls.Config.CipherSuites` for TLS 1.2, unknown names silently skipped, TLS 1.3 ciphers are unaffected); `handshakeTimeout` (decimal seconds → `http.Transport.TLSHandshakeTimeout`); `shareSession = false` (disables TLS session tickets via `tls.Config.SessionTicketsDisabled`); `protocol.versions` (list of `"TLSv1.0"`, `"TLSv1.1"`, `"TLSv1.2"`, `"TLSv1.3"` → `tls.Config.MinVersion`/`MaxVersion`). Accepted at compile time but not implemented at runtime: `certValidation` (OCSP/CRL — not supported in Go stdlib); `sessionTimeout` (not configurable in `crypto/tls`); `keyPassword` in `CertKey` (`tls.X509KeyPair` requires unencrypted PEM — password-protected private keys are unsupported); `protocol.name` (Go only supports TLS; `SSL`/`DTLS` are accepted but ignored). Not supported: `crypto:TrustStore` for `cert` or `crypto:KeyStore` for `key` — both require the `ballerina/crypto` module which is not implemented.
